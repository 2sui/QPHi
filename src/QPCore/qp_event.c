...
qp_event_source_alloc_read_cache(qp_event_t event, qp_event_source_t source) 
{
    ...
    if (!source->read_cache) {
        ...
        QP_LOGOUT_LOG("qp_event_source_alloc_read_cache get memory fail");
        return QP_ERROR;
    }
...
qp_event_source_alloc_write_cache(qp_event_t event, qp_event_source_t source) 
{
    ...
    if (!source->write_cache) {
        ...
        QP_LOGOUT_LOG("qp_event_source_alloc_write_cache get memory fail");
        return QP_ERROR;
    }
    ...
    if (source->read) {
        ...
        if (!source->read_cache) {
            if (QP_ERROR == qp_event_source_alloc_read_cache(event, source)) {
                ...
                source->read = 0;
                QP_LOGOUT_LOG("qp_event_source_read set close by no cache");
                return QP_ERROR;
            }
        }
    ...
    if (source->write) {
        ...
        do {
            ...
            if (1 > ret) {
                ...
                {
                    qp_event_source_set_shutdown(source);
                    QP_LOGOUT_LOG("qp_event_source_write set close by error [%s]", strerror(errno));
                    source->write = 0;
                    return QP_ERROR;
                }
...
{ 
    ...
    if (qp_pool_available(&event->event_pool) && (fd > QP_FD_INVALID)) {
        ...
        QP_LOGOUT_LOG("qp_event_addevent [%d] current available event [%d], used [%d]", \
            source->source_fd, qp_pool_available(&event->event_pool), qp_pool_used(&event->event_pool));
        ...
        do {
            ...
            if (QP_FD_INVALID == sys_accept_fd) {
                if (!(EAGAIN == errno|| EWOULDBLOCK == errno|| EINTR == errno)) {
                    qp_event_removeevent(event, source);
                    QP_LOGOUT_LOG("qp_event_removeevent error [%s].", strerror(errno));
                }
            ...
            {
                close(sys_accept_fd);
                QP_LOGOUT_LOG("qp_event_addevent can not add event.");
            }
...
qp_event_dispatch_queue(qp_event_t event) {
    ...
    while (!qp_list_is_empty(&event->ready)) {
        ...
        if (!source->write_cache && source->write_cache_offset > 0ll) {
